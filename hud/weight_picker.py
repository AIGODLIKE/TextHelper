...
def variants_for_text(context, text_data):
    ...
    variants = ensure_weight_variants(catalog, path)
    if variants:
        return variants
...
def _make_regular_variant(filepath, display_name):
    from ..utils.font_family import FontWeightVariant
    ...
    return FontWeightVariant(
        ...
        display_name=display_name or "Regular",
    )
...
def _invoke_apply_weight(context, filepath, catalog_index, *, keep_picker_open=False):
    from ..utils.text_format import get_active_text
...
def _try_hover_apply_weight(context, filepath, catalog_index=-1):
    ...
    if text_data is not None and is_current_font(text_data, filepath):
        ...
        return False
    if not _invoke_apply_weight(context, filepath, catalog_index, keep_picker_open=True):
        return False
    _HOVER_APPLY_INDEX = catalog_index
    return True
...
def draw_weight_picker(context):
    ...
    for i, hit in enumerate(layout["row_hits"]):
        ...
        blf.color(_UI_FONT, *(theme["text"] if active or hovered else theme["muted"]))
