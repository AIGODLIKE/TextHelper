...
def read_font_name_records(filepath: str) -> tuple[_NameRecord, ...]:
    ...
    for index in range(num_tables):
        ...
        if name_length < 6 or name_offset + name_length > len(data):
            break

        _format_version, count, storage_offset = struct.unpack_from(">HHH", data, name_offset)
        if count <= 0:
            break

        string_base = name_offset + storage_offset

        for rec in range(count):

            rec_off = name_offset + 6 + rec * 12
            ...
            text = _decode_name_record(data, string_base + offset, length, is_unicode=is_unicode)
