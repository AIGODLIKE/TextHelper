...
def _set_editable_raw(text_data, raw: str, *, context=None) -> None:
    ...
    raw = clamp_multiline_text(raw, context=context)
    if is_vertical(text_data):
        assign_vertical_source(text_data.text_helper, raw, context=context)
        from .text_orientation import sync_vertical_source_to_body

        sync_vertical_source_to_body(text_data, context=context)
        return
    assign_text_body(text_data, raw, context=context)
